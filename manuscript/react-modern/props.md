...
Everything that we pass from a parent component to a child component via a component element's HTML attribute can be accessed in the child component. The child component receives as a parameter, an object (`props`) which includes all the passed attributes as properties (props).
